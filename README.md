...
This sample shows the full journey from a monolithic OpenTofu root to a bootstrapped Snap CD project: one big state gets split into per-module states with [demonolith](https://github.com/schrieksoft/demonolith) (0.6.0 or newer), the split is proven to change nothing, and the new modules are then wired into Snap CD, which takes over the ordering and value-passing the monolith used to do implicitly.
...
- **Local child modules** — `./modules/subnet` (called twice) and `./modules/dns`. A module call carries state, so each call has to land in exactly one of the new modules.
...
1. **Decorate and split the code** — `./step3_refactor.sh` (`demonolith split refactor -y --monorepo --engine tofu --overwrite`) runs map → run → validate → diff (`--overwrite` because the module directories from the last run are committed here — run owns them outright and deletes them before rewriting): the map (`demonolith-refactor-map.yaml`) records the whole split for review — including where every module's state will live in the store — `refactor run` executes it, `refactor validate` has the engine check that every written module directory is valid, and `refactor diff` is the CI gate that the committed output still matches the source. The new module directories land in `roots/` (the default) — *roots*, not *modules*, because each is a root module with its own backend, provider initialization, and state, where a reusable module (`modules/` here) by convention has none of those and leaves them to the calling root. `--monorepo` makes the new roots reference `./modules/subnet` and `./modules/dns` by relative path instead of receiving copies. The script sources nothing from `.env`: the validate step is credential-free (it contacts only the provider registry), and every other refactor step is offline and touches nothing but files.
2. **Migrate the state** — `./step4_migrate.sh` (`demonolith split migrate --engine tofu -y --force --var database_port=5432`) runs map → prove → run → verify: pull the monolith's state read-only from the store, back it up, split it into local per-module copies, prove every new module plans to zero changes with producer outputs fed into consumer inputs, push each module's state into its derived bucket key (empty destinations only, never forced — the monolith's own state is never written), and verify the result against the real backends. The input spread is reconstructed on the way through: the store credentials step 2's init resolved flow into gitignored per-module `demono.env` files (as `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` — the s3 backend's own variables), and each module's resolved variable values (tfvars, `TF_VAR_*`, and the re-supplied `-var`) land in its `demono.root.tfvars` and `demono.graph.tfvars`.
3. **Bootstrap into Snap CD** — `refactor` already generated `roots/snapcd/`: a bootstrap module with one `snapcd_module` per module, the map's `cross_edges` realized as `snapcd_module_input_from_output` wirings, its `ordering_edges` as `snapcd_depends_on_module`, and the external inputs passed through as literals bound to the bootstrap's variables — plus, because the split is `--monorepo`, `default_trigger_path_filter_enabled = true` on the namespace, so each module only redeploys when a commit touches its own directory. Apply it (`tofu apply` in `roots/snapcd/` with `source_url` set to this repo), retire the monolith, and Snap CD passes the values between modules at runtime — the same passing the proofs performed locally.

## The same story in CI

[`.github/workflows/migrate.yml`](.github/workflows/migrate.yml) runs the journey as the three lanes a real team would use. It is here for illustration, so the whole workflow targets the `pr-to-me` demo branch and nothing runs on `main`: `refactor diff` as the standing gate on every PR into `pr-to-me` (offline, no credentials), a read-only migration rehearsal on the same PRs (`migrate map` + `migrate prove` — nothing pushed, so a rejected PR leaves the world untouched), and the migration itself only as a manual `workflow_dispatch` on `pr-to-me` — a deliberate, once-off action taken after the merge, not a side effect of CI; it needs no CI pre-gate, because demonolith itself refuses a stale or out-of-sync map. Because this sample has no persistent world, each job first builds its own: the simulated S3 store (`./step0_store.sh` — the same one command as locally, up in seconds) and the baseline apply. In a real repository those steps disappear and the `.env` values come from CI secrets.
